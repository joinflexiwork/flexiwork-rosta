-- ============================================
-- NOTIFICATION SYSTEM MIGRATION (IDEMPOTENT)
-- Handles existing legacy tables from previous failed runs
-- ============================================

-- 0. Cleanup: Drop legacy tables if they exist from previous failed migration
DROP TABLE IF EXISTS public.notifications_legacy CASCADE;
DROP TABLE IF EXISTS public.notification_preferences_legacy CASCADE;

-- Also drop the old tables if they exist (from very old schema)
DROP TABLE IF EXISTS public.notifications CASCADE;
DROP TABLE IF EXISTS public.notification_preferences CASCADE;

-- 1. Create notifications table (Fresh start)
...
-- 4. Drop existing policies (if any) before creating new ones
...
-- 6. Create shift acceptance trigger
...
BEGIN
  ...
  VALUES (v_organisation_id, NULL, v_owner_id, 'shift', 'shift_accepted', v_title, v_body, 'normal');
