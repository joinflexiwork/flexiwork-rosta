...
-- restores minimal owner-based policies so dashboard loads.
-- =============================================================================

-- ========== STEP 1: DROP ALL POLICIES (every known name) ==========
...
-- Shift_allocations (drop so we can recreate minimal)
...
-- ORGANISATIONS: owner only (no team_members read = no recursion)
...
-- VENUES: read organisations (owner_id only, no back ref)
CREATE POLICY "Owners can view venues"
...
-- ROTA_SHIFTS: read venues -> organisations (no back ref)
CREATE POLICY "Owners can manage shifts"
  ON rota_shifts FOR ALL TO authenticated
  USING (
    venue_id IN (
...
CREATE POLICY "Employees can view shifts at venues"
...
-- ROLES: read organisations only
CREATE POLICY "Owners can view roles"
...
-- TEAM_MEMBERS: employees see self (user_id); owners see team (read organisations only)
...
-- SHIFT_ALLOCATIONS
CREATE POLICY "Owners can manage allocations"
