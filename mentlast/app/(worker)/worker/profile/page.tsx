    ...
    async function load() {
      try {
        ...
        const { data: members } = await supabase
          ...
          `)
          .eq('user_id', user.id)
        setMemberships((members as TeamMemberRow[]) ?? [])
      } catch (e) {
        console.error(e)
      } finally {
        setLoading(false)
            ...
            {memberships.map((m) => (
              <li key={m.id} className="border-b border-gray-100 last:border-0 pb-4 last:pb-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="font-medium">{m.primary_venue?.name ?? 'Organisation'}</span>
                </div>
                <div className="text-sm text-gray-600">
                  Roles: {Array.isArray(m.roles) ? m.roles.map((r: { role: { name: string } }) => r.role?.name).filter(Boolean).join(', ') || '—' : '—'}
                </div>
                <div className="text-sm text-gray-500 mt-1">
                  Employment: {m.employment_type?.replace('_', ' ') ?? '—'} • Status:{' '}
                  <span className={m.status === 'active' ? 'text-green-600' : 'text-amber-600'}>
                    {m.status === 'active' ? 'Active' : m.status === 'inactive' ? 'Inactive' : m.status}
                  </span>
                </div>
              </li>
            ))}
